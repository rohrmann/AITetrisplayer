...
public class TLabel {
	...
	public void Arrow(double x, double y, double w, double h, double Scale,
		...
		double[] xarray = {
				x + w / 2 - w / 10 + Scale * 1 / 10
						* Math.sqrt(h * h * 25 + w * w) * Math.sqrt(3) / 3,
				x + w / 2 - w / 10, x + w / 2 - w / 10 };
		double[] yarray = {
				y,
				y + Scale * 1 / 10 * Math.sqrt(h * h * 25 + w * w)
						* Math.sqrt(2) / 2,
				y - Scale * 1 / 10 * Math.sqrt(h * h * 25 + w * w)
						* Math.sqrt(2) / 2 };
		setPenColor(Color);
	...
	public void image(double x, double y, Image image, double w, double h) {
		...
		if (ws <= 1 && hs <= 1)
			...
			offscreen.drawImage(image, (int) Math.round(xs), (int) Math
					.round(ys), (int) Math.round(ws), (int) Math.round(hs),
					null);
		}
	}
	...
	public BufferedImage invert(Image image) {
		BufferedImage b1 = new BufferedImage(image.getWidth(null), image
				.getHeight(null), BufferedImage.TYPE_INT_RGB);
		Graphics bg = b1.getGraphics();
		bg.drawImage(image, 0, 0, null);
		bg.dispose();

		BufferedImage b2 = new BufferedImage(image.getWidth(null), image
				.getHeight(null), BufferedImage.TYPE_INT_RGB);
